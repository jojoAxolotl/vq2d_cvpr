...
def extract_clip_frame_nos_joko(video_md, clip_annotation, save_root):
    """
    Extracts frame numbers corresponding to the VQ annotation for a given clip
    ...
    # if not qset["is_valid"]:
    #     continue
    q_fno = clip_annotation["query_frame"] # query frame
    vc_fno = clip_annotation["visual_crop"]["frame_number"]
    rt_fnos = [rf["frame_number"] for rf in clip_annotation["response_track"]]
    # also add negative frames
    rt_fnos = sorted(
        rt_fnos
    )
...
def video_to_image_fn_joko(inputs):
    ...
    # Create root directory to save clip
    # if video_data["clip_uid"] is None:
    #     continue

    os.makedirs(os.path.join(args.save_root, video_data["clip_uid"]), exist_ok=True)

    # Get list of frames to save
    frame_nos_to_save += extract_clip_frame_nos_joko(video_md, video_data, args.save_root)
...
def main(args):
    ...
    for annot_path in args.annot_paths:
        ...
        annotation_export += json.load(open(annot_path, "r"))
    ...
    _ = list(
        tqdm.tqdm(
            pool.imap_unordered(video_to_image_fn_joko, inputs),
            total=len(inputs),
        )
    )
