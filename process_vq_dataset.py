...
def process_video_annot_jokodatasets(video_name, video_data, n_samples_so_far):
    
    annotations = []

    # split = video_data["split"]

    for clip_annot in video_data["annotations"]:
        for qset_id, qset in clip_annot["query_sets"].items():
            if not qset["is_valid"]:
                continue
...
def process_data(args):
    ...
    for split, split_export in annotation_exports.items():
        ...
        for video_name in tqdm.tqdm(split_export.keys()):
            proc_video_data = process_video_annot_jokodatasets(video_name, split_export[video_name], n_samples)
            split_data += proc_video_data
            n_samples += len(proc_video_data)
